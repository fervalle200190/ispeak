...
export default function CourseNavItems({ courseId, moduleId, bubbleId, materials = [], showBar, secondBar, url, community }) {
     return (
          <ol>
                    ...
                    return completada ? (
                         <li key={id} className={`flex items-center p-2 text-gray-500 ${secondBar? "": "pl-0 pr-0 justify-center"}`}>
                              <Link
                                   ...
                                   href={`/${url}/${courseId}/module/${moduleId}/material/${id}/${community ? bubbleId: ''}`}
                              >
